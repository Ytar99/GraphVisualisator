        ...
        private void UpdateNodes(int[,] conjMatrix)
        {
            ...
            for (int i = 0; i < conjMatrix.GetLength(0); i++)
                for (int j = 0; j < conjMatrix.GetLength(1); j++)
                    if (conjMatrix[i, j] != 0)
                        _nodes[i].Linked.Add(new NodeLink(_nodes[j], conjMatrix[i, j]));
        }

        private void CreateNodes(int[,] conjMatrix)
        {
            string alphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
            _nodes = Enumerable.Range(0, conjMatrix.GetLength(0)).Select(i => new Node(alphabet[i % alphabet.Length].ToString())).ToList();
